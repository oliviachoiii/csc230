...
void pulse_glow(uint8_t LED) {
	for (;;){	//infinite loop
		count = 0;
		slow_count = 0;
		for (int threshold = 0; threshold <= PWM_PERIOD; threshold = slow_count * 0.1){	//change the value of threshold over time
			if (count < PWM_PERIOD)
				led_state(LED,0);
